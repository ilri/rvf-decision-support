...
#From serices.py
from dashboard.services import TemperatureService
from base.services import GEEServices

class TemperatureTimeseries(generics.GenericAPIView):
        ...
        try:
            ...
            if serializer.is_valid():#Validation mentioned into Serializers
                ...
                if clip_data := GEEServices.get_clip_data(self, request, 'geometry'):
                    try:
                        if result := TemperatureService.timeseries(self, request, clip_data):
                            success_response.update(result = {'graph_data': result}, code = status.HTTP_200_OK)
                            return Response(success_response, status = status.HTTP_200_OK)  
                        else:
                            error_response.update(code = status.HTTP_400_BAD_REQUEST, message = messages.ERROR["DATA_NOT_AVAIL"], errors = "")
