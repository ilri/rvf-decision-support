...
class ForecastAccumulatedRainfallCHIRPSMap(generics.GenericAPIView):
            ...
            if serializer.is_valid():#Validation mentioned into Serializers
                try:
                    ...
                    if os.path.exists(file_path) and not request.data.get('adm1_name'):
                        ...
                        if result := AccumulatedRainfallService.forecast_map(self, request, 'accumulated_rainfall_chrips', 'chirps'):
                            ...
                            try:
                                if data := AccumulatedRainfallService.rainfall_data_from_shp(self,request):
                                    result = {'map_data': data, 'legend':constants.RAINFALL_DESCRET_LEGEND}
                                    success_response.update(result = result, code = status.HTTP_200_OK)
                                    return Response(success_response, status = status.HTTP_200_OK)
                            except:
